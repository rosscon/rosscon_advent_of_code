...
def ex_int_input(inputs):
    global intInputParam
    if(len(intInputParams) < (intInputParam + 1)):
        intInput = input("Enter an integer: ")
        return int(intInput)
    else:
        intInput = int(intInputParams[intInputParam])
        intInputParam += 1
        return intInput
...
for combo in permutations(phaseInputs, 5):
    ...
    for phase in list(combo):
        memory = memoryOrig.copy()
        intInputParams = [phase, prevOut]
        execute()
        prevOut = out

    if prevOut > maxSignal:
        maxSignal = prevOut
