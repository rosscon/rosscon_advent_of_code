...
def find_steps_to_point(wire, intersection):

    previousCoordinate = [0,0]
    steps = 0

    intersectionPoint = Point(intersection)


    for p in wire:
        ls = LineString([previousCoordinate, p])

        crosses = (ls.distance(intersectionPoint) == 0)

        if crosses:
            ls = LineString([previousCoordinate, intersectionPoint])
...
def find_steps_to_point_wires(wire1, wire2, intersection):
    return find_steps_to_point(wire1, intersection) + find_steps_to_point(wire2, intersection)
...
for intersection in intersections:
    distances.append(find_steps_to_point_wires(wiresCoordinates[0], wiresCoordinates[1], intersection))
