...
board_won_at_num = [-1 for x in markings]
...
for number in draw_numbers:
    ...
    for i in range(len(boards)):
        ...
        if board_won_at_num[i] == -1 and is_winning_board(markings[i]):
            board_won_at_num[i] = number
            # Last winning number called
            if len([x for x in board_won_at_num if x == -1]) == 0:
